...
class Create_mac(FlaskForm):
    work_order_number = StringField('工單號碼',[ DataRequired(message="盤數不可以空白"), Length(min=8,max=8,message="字數不對!"), Regexp("^[A|B|D|R|J]\d{7}",message="格式錯誤[舉例 : A1712254] ")])

class Update_mac(FlaskForm):
    old_mac_address = StringField('舊的MAC ID',[Regexp("[0-9A-F]{2}\-[0-9A-F]{2}\-[0-9A-F]{2}",message="格式錯誤[舉例 : 0D-01-A1 16進為格式] "), Length(min=8,max=8,message="【Old Mac】字數不對!")])
    new_mac_address = StringField('新的MAC ID',[Regexp("[0-9A-F]{2}\-[0-9A-F]{2}\-[0-9A-F]{2}",message="格式錯誤[舉例 : 0D-01-A1 16進為格式] "), Length(min=8,max=8,message="【New Mac】字數不對!")])
    pollet_index = IntegerField('盤數',[NumberRange(min=0,max=99,message="數字0-99盤")])
    work_order = StringField('工單號碼',[ DataRequired(message="工單不可以空白"), Length(min=8,max=8,message="字數不對!"), Regexp("^[A|B|D|R|J]\d{7}",message="格式錯誤[舉例 : A1712254] ")])
